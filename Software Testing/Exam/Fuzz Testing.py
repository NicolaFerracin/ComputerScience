...
# Write a random fuzzer for a simulated text viewer application
    # empty list collecting the modified strings
    modified_list = []
    # str to bytearray
    b = bytearray(content)
    # modify the string 100 times, and each time modify 200 bytes
    for i in range(0, 100):
        for x in range(0, 200):
            b[random.randrange(0, len(content))] = "%c"%random.randrange(128)
        modified_list.append(str(b))
